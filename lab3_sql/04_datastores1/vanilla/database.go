...
var Connect string = "dbname=assess user=postgres host=localhost port=5432 sslmode=disable"

type Database struct {
	*sql.DB
}

// OpenDatabase attempts to open the database and return a handle to
// it
func OpenDatabase() (*Database, error) {
	db := Database{}
	var err error

	db.DB, err = sql.Open("postgres", Connect)
	if err != nil {
